...
public class OrdersServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        Order newOrder = new Order(bookId, customerId, saleId, dateInput, dateOutput, quantity, totalSum);

        try {
            Long id = orderDao.insert(newOrder);
            System.out.println("Adding order result: " + id);
        } catch (DAOException e) {
            e.printStackTrace();
            request.setAttribute("error", "Ошибка при добавлении заказа: " + e.getMessage());
        }
